...
class ControllerCheckoutGuestStep2 extends Controller {
	private $error = array();
  	...
  	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && isset($this->request->post['voucher']) && $this->validateVoucher()) {
  	...
  	private function validate() {
		if ($this->cart->hasShipping()) {
    		...
    		if (isset($this->request->post['shipping_method']) && empty($this->request->post['shipping_method'])) {
				...
				if ( isset($this->session->data['shipping_methods']) && !empty($shipping[0]) && empty($this->session->data['shipping_methods'][$shipping[0]]['quote'][$shipping[1]])) {
					$this->error['warning'] = $this->language->get('error_shipping');
				}
			}
    	...
    	if (isset($this->request->post['payment_method']) && empty($this->request->post['payment_method']) ) {
			...
			if ( isset($this->session->data['payment_methods']) && isset($this->request->post['payment_method']) &&  empty($this->session->data['payment_methods'][$this->request->post['payment_method']])) {
				$this->error['warning'] = $this->language->get('error_payment');
			}
		}
	  ...
	  private function validateVoucher() {
		// echo $this->request->post['voucher'];exit;
		$this->load->model('total/voucher');

	  $this->language->load('checkout/payment');

	    $voucher = $this->model_total_voucher->getVoucher($this->request->post['voucher']);

	  if (!$voucher) {
			$this->error['warning'] =  "Warning: Gift Voucher is either invalid or the balance has been used up!" ; //$this->language->get('error_coupon');
	  }
