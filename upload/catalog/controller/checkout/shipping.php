...
class ControllerCheckoutShipping extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && !empty($this->request->post['is_products']) && $this->validate()) {
        ...
        if(isset($this->request->post['is_products']) && empty($this->request->post['is_products']) ){
            $this->redirect(HTTPS_SERVER . 'index.php?route=checkout/payment');
        }
...
//        }
        ...
        $this->data['is_products'] = false;
        if(!empty($this->session->data['cart'])){
            $this->data['is_products'] = true;
        }
