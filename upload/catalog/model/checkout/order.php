        ...
        if ($order_query->num_rows) {
            ...
            if (!empty($this->session->data['vouchers'])) {
                foreach ($this->session->data['vouchers'] as $voucher) {
                    ...
                    $this->db->query("INSERT INTO " . DB_PREFIX . "voucher SET order_id = '" . (int) $order_id . "', code = '" . $this->db->escape($code) . "', from_name = '" . $this->db->escape($voucher['from_name']) . "', from_email = '" . $this->db->escape($voucher['from_email']) . "', to_name = '" . $this->db->escape($voucher['to_name']) . "', to_email = '" . $this->db->escape($voucher['to_email']) . "', voucher_theme_id = '" . (int) $voucher['voucher_theme_id'] . "', message = '" . $this->db->escape($voucher['message']) . "', amount = '" . (float) $voucher['amount'] . "', status = '1', date_added = NOW()");
                    $voucher_id = $this->db->getLastId();

                    if (!empty($voucher_id)) {
                        $this->db->query("INSERT INTO " . DB_PREFIX . "order_voucher SET order_id = '" . (int) $order_id . "', voucher_id = '" . $this->db->escape($voucher_id) . "', description = '" . $this->db->escape($voucher['description']) . "', code = '" . $this->db->escape($code) . "', from_name = '" . $this->db->escape($voucher['from_name']) . "', from_email = '" . $this->db->escape($voucher['from_email']) . "', to_name = '" . $this->db->escape($voucher['to_name']) . "', to_email = '" . $this->db->escape($voucher['to_email']) . "', voucher_theme_id = '" . (int) $voucher['voucher_theme_id'] . "', message = '" . $this->db->escape($voucher['message']) . "', amount = '" . (float) $voucher['amount'] . "'");
                                           
                    }
                }

                if (!empty($voucher_id)) {
                    // vouchers session data remove
            ...
            if(isset($this->session->data['voucher']) && !empty($this->session->data['voucher']) ){
                ...
                $this->model_total_voucher->confirm($order_query, $order_total_query);
            }
            ...
            if (!empty($vouchers)) {
                foreach ($vouchers as $voucher) {
                    $template->data['vouchers'] = array(
                        ...
                        'amount' => $this->currency->format($voucher['amount'], $order_query->row['currency'], $order_query->row['value'])
                    );
                }
            }
