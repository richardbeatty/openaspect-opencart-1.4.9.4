...
class ControllerSaleOrder extends Controller {
    ...
    private function getList() {
        ...
        foreach ($results as $result) {
            $action = array();
            /* code to check if there is voucher in order */
            //$sql = "SELECT `voucher_id` FROM `voucher` WHERE `order_id` = 12";
    ...
    public function getForm() {
        ...
        $v_info = $this->model_sale_voucher->getVoucherByOrder($order_id);
        ...
        if ($order_info) {
            ...
            foreach ($products as $product) {
                ...
                if ($v_info) {
                    //echo " I m here".$v_info['voucher_id']; exit;
                    $this->data['products'][] = array(
                        ...
                        'href' => HTTPS_SERVER . 'index.php?route=sale/voucher/update&token=' . $this->session->data['token'] . '&voucher_id=' . $v_info['voucher_id']
                    );
                } else {
                    $this->data['products'][] = array(
                        'order_product_id' => $product['order_product_id'],
            ...
            if (!empty($vouchers)) {
                foreach ($vouchers as $voucher) {
                    $this->data['vouchers'][] = array(
                        ...
                        'quantity' => 1,
                        'amount' => $this->currency->format($voucher['amount'], $order_info['currency'], $order_info['value'])
                    );
                }
            }
